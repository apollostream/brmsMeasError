// generated with brms 2.14.4 and manually-edited by M.L. Thompson
functions {
}
data {
...
model {
  ...
  if (prior_only) {
    // EXPLICIT PRIORS FOR Ymi_x and Ymi_y.
    target += normal_lpdf(Ymi_x | mu_x, sigma_x);
    target += normal_lpdf(Ymi_y | mu_y, sigma_y);
  } else {
    // DUAL ROLE STATMENTS: 
    // Implicitly place priors on Ymi_x & Ymi_y, thru Yl_x[Jmi_x] & Yl_y[Jmi_y],
    // which have missing values; while serving as likelihoods using the remaining
    // Yl_x & Yl_y, which have observations.
